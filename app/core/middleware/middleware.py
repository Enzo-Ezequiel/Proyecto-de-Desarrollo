    ...
    async def __call__(self, scope: Scope, receive: Receive, send: Send) -> None:
        ...
        if content_length is not None:
            try:
                ...
                request_size = None
                logger.warning(
                    "Se recibió un encabezado content-length con formato inválido."
                )

            if request_size is not None and request_size > self.max_size_bytes:
                logger.warning(
                    f"Solicitud HTTP rechazada: El tamaño ({request_size} bytes) "
                    f"excede el límite de {self.max_size_bytes} bytes."
                )
